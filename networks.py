...
class Encoder(nn.Module):
    def __init__(self, style_dim, class_dim):
        ...
        self.linear_model = nn.Sequential(OrderedDict([
            ('linear_1', nn.Linear(in_features=1, out_features=4, bias=True)),
            ('tan_h_1', nn.Tanh())
        ]))

        # style
        self.style_mu = nn.Linear(in_features=4, out_features=style_dim, bias=True)
        self.style_logvar = nn.Linear(in_features=4, out_features=style_dim, bias=True)

        # class
        self.class_mu = nn.Linear(in_features=4, out_features=class_dim, bias=True)
        self.class_logvar = nn.Linear(in_features=4, out_features=class_dim, bias=True)
...
class Decoder(nn.Module):
    def __init__(self, style_dim, class_dim):
        ...
        self.linear_model = nn.Sequential(OrderedDict([
            ('linear_1', nn.Linear(in_features=style_dim + class_dim, out_features=4, bias=True)),
            ('tan_h_1', nn.Tanh()),

            ('linear_2', nn.Linear(in_features=4, out_features=1, bias=True))
            # ('sigmoid_final', nn.Sigmoid())
        ]))
        # self.linear_model = self.linear_model.to('cuda')
...
if __name__ == '__main__':
    ...
    loader = cycle(DataLoader(mnist, batch_size=64, shuffle=True, drop_last=True))
